...
for a in range(1, 7):
    ...
    print(time.year, '/', time.month, '/', time.day,
          ...
          "降雨量", preds[a][3],
          "风力", preds[a][4])
    weather_dao.update_predict_byid(int(a),
                                    numpy.float(preds[a][0]),
                                    numpy.float(preds[a][1]),
                                    numpy.float(preds[a][2]),
                                    numpy.float(preds[a][3]),
                                    numpy.float(preds[a][4]))
...
plt.xlabel("day")
# 显示
plt.savefig('D:\\phython\\pythonProjects\\weatherDataView\\plot.png')
plt.show()
