...
# 这里为数据筛选 只留几个关键和其二级城市。
def data_filter():
    child = []
    for index, i in enumerate(data_old):
        child.append({"provinceName": i["provinceName"], "provinceShortName": i["provinceShortName"]
                         , "locationId": i["locationId"]})
        childs = []
        for j in i["cities"]:
            childs.append({"cityName": j["cityName"], "locationId": j["locationId"]})
        child[index].update({"cities": childs})
    # print(json.dumps(child ,indent=1 ,ensure_ascii=False))
    return json.dumps(child, indent=1, ensure_ascii=False)


if __name__ == '__main__':
    main_city = data_filter()
    print(main_city)